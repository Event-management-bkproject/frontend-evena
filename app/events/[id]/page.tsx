...
import {
  Box, Container, Typography, Button, Chip, CircularProgress, Alert, Divider,
} from '@mui/material';
import {
  CalendarToday, Place, AccessTime, Group, ArrowBack,
  CheckCircleOutline, People, LockOutlined,
...
export default function PublicEventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Use ticket types embedded in EventResponse (no extra auth-required call)
  const ticketTypes = (event?.ticketTypes ?? []).filter(
    (t) => t.status === TicketTypeStatus.ACTIVE || t.status === TicketTypeStatus.SOLD_OUT,
  );
  ...
  const handleBuyTickets = () => {
    const dest = `/dashboard/customer/events/${eventId}/tickets`;
    if (!auth.accessToken) {
    ...
    return (
      <Box sx={{ bgcolor: '#F8FAFC', minHeight: '100vh' }}>
        ...
        <Container maxWidth="lg" sx={{ py: 8 }}>
          <Alert severity="error">Event not found or unavailable.</Alert>
          <Button startIcon={<ArrowBack />} onClick={() => router.push('/events')} sx={{ mt: 2, textTransform: 'none' }}>
            Back to Events
          </Button>
        </Container>
        <Footer />
  ...
  return (
    <Box sx={{ bgcolor: '#F8FAFC', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ position: 'relative', width: '100%', height: { xs: 280, sm: 380, md: 480 }, overflow: 'hidden' }}>
        <Box component="img" src={event.coverUrl || FALLBACK_IMG} alt={event.title}
        ...
        <Box sx={{ position: 'absolute', top: 20, left: { xs: 16, md: 40 }, zIndex: 2 }}>
          <Button startIcon={<ArrowBack />} onClick={() => router.push('/events')}
            sx={{ color: '#fff', bgcolor: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(8px)', borderRadius: '10px', textTransform: 'none', fontWeight: 600, fontSize: 13, px: 2, py: 0.75, '&:hover': { bgcolor: 'rgba(0,0,0,0.55)' } }}>
            All Events
          </Button>
        </Box>

        <Box sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, p: { xs: 3, md: 5 }, zIndex: 2 }}>
          {event.category && (
            <Chip label={event.category.name} size="small"
              sx={{ bgcolor: 'rgba(243,107,249,0.85)', color: '#fff', fontWeight: 700, fontSize: 11, mb: 1.5 }} />
          )}
          <Typography variant="h3"
            sx={{ color: '#fff', fontWeight: 800, fontSize: { xs: '1.6rem', sm: '2.2rem', md: '2.8rem' }, lineHeight: 1.2, textShadow: '0 2px 16px rgba(0,0,0,0.4)', mb: 1.5, maxWidth: 720 }}>
            {event.title}
          </Typography>
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            {[
              { icon: <CalendarToday sx={{ fontSize: 15 }} />, label: fmt(event.startAt) },
              { icon: <Place sx={{ fontSize: 15 }} />, label: `${event.venue?.name}, ${event.venue?.city}` },
            ].map(({ icon, label }) => (
              <Box key={label} sx={{ display: 'flex', alignItems: 'center', gap: 0.75, color: 'rgba(255,255,255,0.85)' }}>
                {icon}
                <Typography variant="caption" sx={{ fontSize: 13, fontWeight: 500 }}>{label}</Typography>
              </Box>
            ))}
          </Box>
        </Box>
      ...
      <Box sx={{ flex: 1, py: { xs: 4, md: 6 } }}>
        <Container maxWidth="lg">
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 360px' }, gap: { xs: 4, lg: 5 }, alignItems: 'start' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <Box sx={{ bgcolor: '#fff', borderRadius: '16px', p: { xs: 3, md: 4 }, boxShadow: '0 2px 12px rgba(0,0,0,0.05)', border: '1px solid #F1F5F9' }}>
                <Typography variant="h6" sx={{ fontWeight: 800, color: '#0F172A', mb: 3, fontSize: 18 }}>Event Details</Typography>
              ...
              {/* About */}
              <Box sx={{ bgcolor: '#fff', borderRadius: '16px', p: { xs: 3, md: 4 }, boxShadow: '0 2px 12px rgba(0,0,0,0.05)', border: '1px solid #F1F5F9' }}>
                <Typography variant="h6" sx={{ fontWeight: 800, color: '#0F172A', mb: 2.5, fontSize: 18 }}>About This Event</Typography>
                <Typography variant="body1" sx={{ color: '#475569', lineHeight: 1.85, fontSize: 15, whiteSpace: 'pre-wrap' }}>
                  {event.description || `Experience an unforgettable ${event.category?.name ?? 'event'} at ${event.venue?.name}.`}
                </Typography>

                {event.imageUrls && event.imageUrls.length > 0 && (
                  <>
                    <Divider sx={{ my: 3, borderColor: '#CBD5E1' }} />
                    <Typography variant="h6" sx={{ fontWeight: 800, color: '#0F172A', mb: 2.5, fontSize: 18 }}>Gallery</Typography>
                    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 1.5 }}>
                      {event.imageUrls.map((url, i) => (
                        <Box key={i} component="img" src={url} alt={`${event.title} ${i + 1}`}
                          sx={{ width: '100%', height: 120, objectFit: 'cover', borderRadius: '10px', display: 'block', transition: 'transform 0.2s', '&:hover': { transform: 'scale(1.03)' } }} />
                      ))}
                    </Box>
                  </>
                )}
              ...
              <Box sx={{ bgcolor: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.10)', border: '1px solid #F1F5F9' }}>
                ...
                <Box sx={{ px: 3, pt: 3, pb: 2 }}>
                  ...
                  <Typography sx={{ fontSize: 32, fontWeight: 900, background: 'linear-gradient(135deg,#F36BF9,#8E2DE2)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 1.1 }}>
                    {ticketTypes.length === 0 ? '—' : fmtPrice(minPrice)}
                  </Typography>
                  {totalAvailable > 0 && (
                ...
                <Box sx={{ px: 3, py: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <Typography variant="body2" sx={{ fontWeight: 700, color: '#0F172A', fontSize: 14 }}>Ticket Types</Typography>

                  {ticketTypes.length === 0 ? (
                    <Box sx={{ py: 3, textAlign: 'center' }}>
                      <Typography variant="body2" color="#94A3B8">No tickets available yet.</Typography>
                    </Box>
                  ) : (
                    ticketTypes.map((ticket) => {
                      const soldPct = ticket.total > 0 ? (ticket.sold / ticket.total) * 100 : 0;
                      const almostGone = ticket.available > 0 && soldPct >= 80;

                      return (
                        <Box key={ticket.id}
                          sx={{ p: 2, borderRadius: '12px', border: '1.5px solid #F1F5F9', transition: 'border-color 0.15s, box-shadow 0.15s', '&:hover': { borderColor: '#F36BF9', boxShadow: '0 4px 16px rgba(243,107,249,0.12)' } }}>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                            <Typography variant="body2" sx={{ fontWeight: 700, color: '#0F172A', fontSize: 14 }}>
                              {ticket.name}
                            </Typography>
                            <Typography sx={{ fontWeight: 900, fontSize: 18, background: 'linear-gradient(135deg,#F36BF9,#8E2DE2)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                            ...
                            {ticket.available > 0 ? (
                              <>
                                ...
                                <Typography variant="caption" sx={{ fontSize: 11, fontWeight: 600, color: almostGone ? '#F59E0B' : '#22C55E' }}>
                                  {almostGone ? `Only ${ticket.available} left!` : `${ticket.available} available`}
                                </Typography>
                              </>
                            ) : (
                          ...
                          <Box sx={{ height: 3, borderRadius: 2, bgcolor: '#F1F5F9', overflow: 'hidden' }}>
                            <Box sx={{ height: '100%', width: `${soldPct}%`, borderRadius: 2, background: almostGone ? 'linear-gradient(90deg,#F59E0B,#EF4444)' : 'linear-gradient(90deg,#F36BF9,#8E2DE2)', transition: 'width 0.4s' }} />
                          </Box>
                        </Box>
                      );
                    })
                ...
                {ticketTypes.length > 0 && (
                  <Box sx={{ px: 3, pb: 3 }}>
                    <Button fullWidth variant="contained" disabled={isSoldOut} onClick={handleBuyTickets}
                      startIcon={!auth.accessToken ? <LockOutlined sx={{ fontSize: 18 }} /> : undefined}
                      sx={{
                        borderRadius: '12px', textTransform: 'none', fontWeight: 800, fontSize: 16, py: 1.75,
                      ...
                      {isSoldOut ? 'Sold Out' : auth.accessToken ? 'Select Tickets →' : 'Login to Buy Tickets'}
                    </Button>
                    {!isSoldOut && (
                      <Typography variant="caption" sx={{ display: 'block', textAlign: 'center', color: '#94A3B8', fontSize: 11, mt: 1 }}>
                        {auth.accessToken ? 'No hidden fees · Instant confirmation' : 'Free account required · Instant confirmation'}
                      </Typography>
                    )}
                  </Box>
                )}
      ...
      {/* Mobile sticky CTA */}
      {ticketTypes.length > 0 && (
        <Box sx={{ display: { xs: 'block', lg: 'none' }, position: 'fixed', bottom: 0, left: 0, right: 0, p: 2, bgcolor: '#fff', borderTop: '1px solid #F1F5F9', boxShadow: '0 -4px 20px rgba(0,0,0,0.08)', zIndex: 100 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Button fullWidth variant="contained" disabled={isSoldOut} onClick={handleBuyTickets}
              startIcon={!auth.accessToken ? <LockOutlined sx={{ fontSize: 16 }} /> : undefined}
              sx={{ borderRadius: '12px', textTransform: 'none', fontWeight: 700, fontSize: 15, py: 1.5, background: 'linear-gradient(135deg,#F36BF9,#8E2DE2)', boxShadow: '0 4px 14px rgba(243,107,249,0.35)', '&:hover': { background: 'linear-gradient(135deg,#e055e8,#7d26d1)' } }}>
              {isSoldOut ? 'Sold Out' : auth.accessToken ? 'Book Now' : 'Login to Buy'}
            </Button>
          </Box>
        </Box>
      )}
