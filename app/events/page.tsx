...
export default function PublicEventsPage() {
  ...
  const { data, isLoading, isFetching } = useGetEventsSearchQuery(searchParams);
  ...
  const handleTimePeriod = (_: React.MouseEvent<HTMLElement>, val: TimePeriod) => {
    if (!val) return;
    setTimePeriod(val);
    setPage(0);
  };
  ...
  return (
    <Box sx={{ backgroundColor: '#F8FAFC', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Hero */}
      <Box sx={{ background: 'linear-gradient(135deg, #ED4690 0%, #5522CC 100%)', color: 'white', py: { xs: 5, md: 7 }, pb: { xs: 9, md: 11 }, position: 'relative', overflow: 'hidden' }}>
        <Box sx={{ position: 'absolute', top: -60, right: '10%', width: 320, height: 320, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <Typography variant="h3" sx={{ fontWeight: 800, mb: 1.5, fontSize: { xs: '2rem', md: '2.8rem' } }}>
            Upcoming Events
          </Typography>
          <Typography sx={{ opacity: 0.85, fontSize: { xs: '1rem', md: '1.1rem' } }}>
            Discover concerts, festivals, workshops and more happening near you
          </Typography>
        </Container>
      </Box>
        ...
        <Collapse in={showFilters}>
          <Box sx={{ bgcolor: '#fff', borderRadius: '0 0 16px 16px', boxShadow: '0 4px 24px rgba(0,0,0,0.07)', border: '1px solid #F1F5F9', borderTop: 'none', p: 3, mt: -0.5 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2, flexDirection: { xs: 'column', sm: 'row' }, mb: 2.5 }}>
              ...
              <ToggleButtonGroup
                value={timePeriod} exclusive onChange={handleTimePeriod} size="small"
              ...
              >
                {([['all', 'All time'], ['today', 'Today'], ['week', 'This week'], ['month', 'This month']] as [TimePeriod, string][]).map(([val, label]) => (
                  <ToggleButton key={val} value={val} disableRipple sx={{
                    ...
                    '&.Mui-selected': { background: 'linear-gradient(135deg,#F36BF9,#6093FC)', color: '#fff', boxShadow: '0 3px 10px rgba(96,147,252,0.3)', '&:hover': { background: 'linear-gradient(135deg,#e055e8,#4a7ef0)' } },
                    '&:hover': { bgcolor: '#F1F5F9' },
                  }}>{label}</ToggleButton>
                ))}
              </ToggleButtonGroup>
            ...
            <Box sx={{ display: 'flex', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2, flexDirection: { xs: 'column', sm: 'row' }, mb: 2.5 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                ...
                {categories.map((cat) => (
                  <Chip key={cat.id} label={cat.name} onClick={() => handleCategoryClick(cat.id)} sx={chipSx(selectedCategory === cat.id)} />
                ))}
              </Box>
            </Box>
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
                  <TextField
                    ...
                    InputProps={{ startAdornment: <InputAdornment position="start"><Typography fontSize={12} color="#94A3B8">₫</Typography></InputAdornment> }}
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <FormControl size="small" sx={{ minWidth: 140 }}>
                    <Select
                      value={sortBy}
                      onChange={(e) => handleSortChange(e.target.value as SortBy, sortDir)}
                      sx={{ fontSize: 13 }}
                    >
                  ...
                  <FormControl size="small" sx={{ minWidth: 110 }}>
                    <Select
                      value={sortDir}
                      onChange={(e) => handleSortChange(sortBy, e.target.value as SortDir)}
                      sx={{ fontSize: 13 }}
                    >
        ...
        {!isLoading && !isFetching && events.length === 0 && (
          <Box sx={{ textAlign: 'center', py: 12 }}>
            ...
            {hasActiveFilters && (
              <Box
                onClick={handleReset}
                sx={{ mt: 2, display: 'inline-block', px: 3, py: 1, borderRadius: '10px', cursor: 'pointer', border: '1px solid #E2E8F0', color: '#6093FC', fontWeight: 600, fontSize: 14, '&:hover': { bgcolor: '#F8FAFC' } }}
              >
                Clear filters
              </Box>
            )}
